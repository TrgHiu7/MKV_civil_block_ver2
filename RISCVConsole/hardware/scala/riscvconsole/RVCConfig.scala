...
/*class WithKECCAK extends Config((site, here, up) => {
  case PeripheryKECCAKKey => Seq(
    KECCAKAttachParams(
      keccakparams = KeccakParams(address = BigInt("4000", 16))
    )
  )
})*/

class WithMKV extends Config((site, here, up) => {
  case PeripheryMKVKey => Seq(
    MKVAttachParams(
      device = MKVParams(address = BigInt("10007000", 16))
    )
  )
})
